...
def plot_metrics_for_each_method(df, methods ,d):
    ...
    for i, method in enumerate(methods):
        ...
        axes[i].set_xlabel('Number of covariate points')
    ...
    plt.savefig('fixbudgetd{}.png'.format(d))
    #plt.show()   
...
def picture_plot1(df):
    ...
    for i, column in enumerate(df):
        ...
        axes[i].set_xlabel('Number of covariate points', fontsize=15)
...
if __name__ == '__main__':
    ...
    for covariate_dim in covariate_dim1:
        ...
        for i,n in enumerate(n_values):
            T = total_budget // n
            h = (total_budget**(-1/(covariate_dim+2)))
        ...
        for i,n in enumerate(n_values):
            T = total_budget // n
            k = int(total_budget**(1/(covariate_dim+2)))
            predicted_theta_values =[]
            for i in range(replication):
                covariates_points, theta_estimates = offline_stage(n, T, eta_0, gamma1, covariate_dim)
        ...
        df = prepare_data_for_plot(n_values, mse_knn, mse_krr, mse_ks,mse_lr)
    
        #df['log2_n'] = np.log2(df['n'])

        data1.append(df)
        df = prepare_plot_data(n_values, mse_knn, variance_knn, bias_knn, mse_krr, variance_krr, bias_krr,mse_ks, variance_ks, bias_ks,mse_lr, variance_lr, bias_lr)
