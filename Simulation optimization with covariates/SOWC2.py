...
def grad_f(theta, x, num_samples):
    ...
    xi_samples = np.random.normal(0, 16, num_samples)  # 随机样本
...
def grad_f(theta, x , num_samples):
    ...
    for rep in range(num_samples):
        ...
        for i in range(d):
            gradients[rep, i] = 2 * (i + 1) * (theta[i] - np.sqrt(x[i])) + noise[i]
...
if __name__ == '__main__':
    ...
    gamma1 = 0.01  # 学习率衰减率
    covariate_dim = 2
    num_samples = 100  # Number of samples for true theta calculation
    ...
    n_values = [int(x**(2/3))for x in total_budget]
    T_values = [int(x**(1/3))for x in total_budget]
    for i,n in enumerate(n_values):
        covariates_points = grid_sample(covariate_dim, lowbound, upbound, n)
        #T = total_budget // n
        T = T_values[i]
    ...
    sns.lineplot(data=df, x='n', y='MSE', hue='Method', style='Method',markers=[ 'o','s', 'D', '^'], dashes=False, markersize=10,linewidth=3)
    plt.xscale('log', base=2)
    
    plt.xlabel('Total budget ', fontsize=20)
    plt.ylabel(r'$log_{2}(MSE)$' , fontsize=20)
    plt.title(r'$d_x = {} $'.format(covariate_dim), fontsize=20)
    #plt.title('MSE vs Total budget for different methods', fontsize=14)
    plt.xticks(fontsize = 20)
    plt.yticks(fontsize = 20)
    plt.grid(True)

    plt.text(0.95, 0.05, f'Covariate Dimension: {covariate_dim}', 
