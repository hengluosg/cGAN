...
def train_nn_surface_model(X, thetas, objective_values, x_dim, epochs=150, lr=1e-3, batch_size=32):
    # 创建训练数据
...
def krr_fit_and_predict(x_train, w_train, mu_train, sigma_train, x_test, lambda_param=1e-4, length_scale=1.0):
    """
    利用 KRR 方法分别拟合高维 w, mu, sigma,并对测试点 x_test 进行预测。
    """
...
def knn_predict(x_train, w_train, mu_train, sigma_train, x_new,surface_model, k=5):
   
    distances = pairwise_distances(x_train, x_new.reshape(1, -1)).flatten()
    
    #distances = cdist([x_new], x_train.reshape(1, -1), 'euclidean')
    # 找到最近的 k 个样本索引
    knn_indices = np.argsort(distances)[:k]
    print(knn_indices)
    # 从训练集中提取 k 个最近邻样本
    w_neighbors = w_train[knn_indices]  # (k, n_components)
    mu_neighbors = mu_train[knn_indices]  # (k, n_components, dim_mu)
    sigma_neighbors = sigma_train[knn_indices]  # (k, n_components, dim_mu, dim_mu)
    
    # 对最近邻的权重、均值、协方差进行加权平均
    # weights = 1 / (distances[knn_indices] + 1e-8)  # 防止除以 0
    # weights /= np.sum(weights)  # 归一化
    
    # # 加权平均
    # w_pred = np.average(w_neighbors, axis=0, weights=weights)  # (n_components,)
    # mu_pred = np.average(mu_neighbors, axis=0, weights=weights)  # (n_components, dim_mu)
    # sigma_pred = np.average(sigma_neighbors, axis=0, weights=weights)  # (n_components, dim_mu, dim_mu)
    
    w_pred = np.average(w_neighbors, axis=0)  # (n_components,)
    mu_pred = np.average(mu_neighbors, axis=0)  # (n_components, dim_mu)
    sigma_pred = np.average(sigma_neighbors, axis=0)  # (n_components, dim_mu, dim_mu)
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        mu_train.append(gmm.means_)
        w_train.append(gmm.weights_)
        sigma_train.append(gmm.covariances_)
        
    w_train = np.array(w_train)
    mu_train = np.array(mu_train)
    sigma_train = np.array(sigma_train)
    print(w_train.shape,mu_train.shape, sigma_train.shape)
    ...
    x_new = np.array([1.5, 1.0]).reshape(1,covariate_dim) # 新的输入
    ...
    x_new = np.array([1.5, 1.0])

    #test_points = np.random.normal(loc=1.25, scale=0.2, size=100)
    
    

    best_theta, generated_theta =  knn_predict(covariates_points, w_train, mu_train, sigma_train, x_new,nn_model, k=10)
