...
def plot_theta_distribution1(x, y, z):
    ...
    for i in range(d):
        # 绘制真实的 θ 的分布
        sns.histplot(x[:, i], bins=20, kde=True, ax=axes[i], color='blue', stat='density', alpha=0.7, label='True', fill=True)
        sns.histplot(y[:, i], bins=20, kde=True, ax=axes[i], color='red', stat='density', alpha=0.7, label='Generate-GMM', fill=True)
        sns.histplot(z[:, i], bins=20, kde=True, ax=axes[i], color='green', stat='density',alpha=0.7, label='Generate-CWGAN', fill=True)
