...
def gradient(theta, x):
    ...
    noise = np.random.normal(0, 1, covariate_dim)
...
def first_stage(x, K, T, eta, d):
    ...
    for i in range(n):
        ...
        for k in range(K):
            ...
            optimized_thetas.append(theta)  # Store the final theta after T steps
...
if __name__ == '__main__':
    ...
    def generate_theta_from_gmm(x_new, gmm, num_samples=10):
        ...
        """
        # 计算每个成分的责任度 (E-step)
        responsibilities = gmm.predict_proba([x_new])[0]  # shape: (K, )
