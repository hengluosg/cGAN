...
def offline_training(gmms, n_clusters):
    ...
    print(cluster_info["weights"].shape)
    return cluster_info, kmeans




def knn_predict1(covariates_points,x_new, gmm_models, n_components1,surface_model,k =5):
    ...
    # all_weights = np.array(all_weights)[knn_indices]  # (k, n_components)
    # all_means = np.array(all_means)[knn_indices]  # (k, n_components, dim_mu)
    # all_covariances = np.array(all_covariances)[knn_indices]  # (k, n_components, dim_mu, dim_mu)
    ...
    n_total_components = all_means.shape[0]

    kmeans = KMeans(n_clusters=n_components1, random_state=42, n_init="auto")
    labels = kmeans.fit_predict(all_means)
    ...
    # for i, label in enumerate(labels):
    #     plt.text(all_means[i, 0] + 0.05, all_means[i, 1] + 0.05, f' {label}', color='black', fontsize=6)
    # plt.scatter(cluster_centers[:, 0], cluster_centers[:, 1], c='red', s=20, marker='o', label='Cluster Centers')
    ...
    for i in range(n_components1):
        ...
        cluster_means = all_means[cluster_indices]
        cluster_weights = np.array([all_weights[j // len(gmm.means_)][j % len(gmm.means_)] 
                                    for j in cluster_indices])
        cluster_covariances = np.array([all_covariances[j // len(gmm.means_)][j % len(gmm.means_)] 
                                        for j in cluster_indices])
        
        
        
        mean_avg = np.mean(cluster_means, axis=0)
        weight_avg = np.sum(cluster_weights) / n_total_components  
        cov_avg = np.mean(cluster_covariances, axis=0)
        new_means.append(mean_avg)
        new_weights.append(weight_avg)
        new_covariances.append(cov_avg)
    ...
    w_pred, mu_pred, sigma_pred = np.array(new_weights), np.array(new_means), np.array(new_covariances)
...
if __name__ == '__main__':
    ...
    z_dim = 10  # 噪声维度
    M = 1000
    A = generate_symmetric_matrix(covariate_dim, correlation_strength=0.1)
    ...
    best_theta, generated_theta = knn_predict1(covariates_points,x_new, gmm_models, n_components1,None,k =10)
    ...
    for x_new in test_points:
        ...
        best_theta, generated_theta =  knn_predict1(covariates_points,x_new, gmm_models, n_components1,nn_model,k =5)
