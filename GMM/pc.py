...
def objective_function(theta, x):
    return np.sum(np.cos(2 *theta) + x * np.sin(2 *theta) + 0.1 * theta**2)
...
def gradient(theta, x):
    ...
    grad_term1 = -2 * np.sin(2 * theta) + 2 * x * np.cos(2 * theta) + 0.2 * theta
...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        best_k_bic, best_k_aic = gmm_bic_aic(X, max_k=10)
    ...
    best_theta, generated_theta =  knn_predict(covariates_points, w_train, mu_train, sigma_train, x_new,nn_model, k=5)
