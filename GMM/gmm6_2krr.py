...
def krr_cross_validation(covariates_points, theta_estimates, lambda_vals=[0.0001, 0.001, 0.01, 0.1, 1, 10], n_splits=6, length_scale=1.0):
...
def krr_online_stage(covariates_points, theta_estimates, x, K_phi_inv, length_scale=1.0):
    ...
    x =x.reshape(1,covariate_dim)
    k_phi_x = gaussian_kernel_krr(x, covariates_points, length_scale).flatten()
    theta_x = np.dot(k_phi_x, K_phi_inv) @ theta_estimates
    return theta_x
def krr_prediction(kmeans_model, all_means, all_weights, all_covariances,surface_model, x_new,lambda_param=1e-4, length_scale=1.0):
    
    #print(all_means.shape,all_weights.shape,all_covariances.shape)
    k, n_components, dim_mu = all_means.shape
    X_means = all_means.reshape(-1, dim_mu)  # (k * n_components, dim_mu)
    predicted_labels = kmeans_model.predict(X_means)  # (k * n_components,)
    ...
    for label in unique_labels:
        ...
        # x_cluster = covariates_points[cluster_indices[0]]
        cluster_means = all_means[cluster_indices]
        # x_cluster = cluster_means
        x_cluster = covariates_points[cluster_indices[0]]
        cluster_weights = all_weights[cluster_indices]
        cluster_covariances = all_covariances[cluster_indices]
        
        y_means,y_weights,y_covariances = cluster_means,cluster_weights,cluster_covariances
        


        y_covariances = y_covariances.reshape(y_covariances.shape[0],-1)
        

        #print(x_cluster.shape,y_means.shape,y_weights.shape,y_covariances.shape)
        
        best_lambda = krr_cross_validation(x_cluster, y_weights, lambda_vals=[ 0.001, 0.01, 0.1, 1])
        K_phi_inv = compute_inverse_kernel_matrix(x_cluster, lambda_param=best_lambda, length_scale = 1)
        predicted_y_weights = krr_online_stage(x_cluster, y_weights, x_new, K_phi_inv, length_scale = 1) 
        

        best_lambda = krr_cross_validation(x_cluster, y_means, lambda_vals=[ 0.001, 0.01, 0.1, 1])
        K_phi_inv = compute_inverse_kernel_matrix(x_cluster, lambda_param=best_lambda, length_scale = 1)
        predicted_y_means = krr_online_stage(x_cluster, y_means, x_new, K_phi_inv, length_scale = 1) 
        
        best_lambda = krr_cross_validation(x_cluster, y_covariances, lambda_vals=[ 0.001, 0.01, 0.1, 1])
        K_phi_inv = compute_inverse_kernel_matrix(x_cluster, lambda_param=best_lambda, length_scale = 1)
        predicted_y_covariances = krr_online_stage(x_cluster, y_covariances, x_new, K_phi_inv, length_scale = 1) 
        predicted_y_covariances = predicted_y_covariances.reshape(dim_mu, dim_mu)
        epsilon = 1e-3  
        predicted_y_covariances += np.eye(predicted_y_covariances.shape[0]) * epsilon
        #print(predicted_y_covariances)
        # print("predicted_y_weights",predicted_y_weights)
        # print("predicted_y_means",predicted_y_means)
        # print("predicted_y_covariances",predicted_y_covariances)
        
    
        averaged_means.append(predicted_y_means)
        averaged_weights.append(predicted_y_weights)
        averaged_covariances.append(predicted_y_covariances)
    ...
    mu_pred, w_pred,  sigma_pred = averaged_means, averaged_weights, averaged_covariances
    ...
    best_idx = np.argmin(scores)
    optimal_theta = samples[best_idx]
    print("KRR-GMM:Generated optimal solution with minimum score:", min(scores))
...
def knn_predict1(kmeans_model,covariates_points,x_new, gmm_models, n_components1,surface_model,k =5):
    ...
    best_idx = np.argmin(scores)
    optimal_theta = samples[best_idx]
    print("KNN-GMM:Generated optimal solution with minimum score:", min(scores))
...
def linear_regression_train(x_train, theta_train, lambda_reg=0.01):
    ...
    return beta_hat
def cross_validate(x_train, theta_train, lambda_vals=[0.0001,0.001, 0.01, 0.1, 1, 10], k_folds=5):
...
def linear_regression_predict(kmeans_model, all_means, all_weights, all_covariances,surface_model, x_new):
    ...
    for label in unique_labels:
        ...
        beta_hat  = cross_validate(x_cluster, y_weights)
        predicted_y_weights = linear_regression_on_stage(x_new, beta_hat)
        
        
        beta_hat = cross_validate(x_cluster, y_means)
        predicted_y_means = linear_regression_on_stage(x_new, beta_hat)

        beta_hat = cross_validate(x_cluster, y_covariances)
        predicted_y_covariances = linear_regression_on_stage(x_new, beta_hat)
        predicted_y_covariances = predicted_y_covariances.reshape(dim_mu, dim_mu)
        epsilon = 1e-3  
        predicted_y_covariances += np.eye(predicted_y_covariances.shape[0]) * epsilon
        #print(predicted_y_covariances)
    ...
    best_idx = np.argmin(scores)
    optimal_theta = samples[best_idx]
    print("LR-GMM:Generated optimal solution with minimum score:", min(scores))
...
if __name__ == '__main__':
    ...
    z_dim = 10  # 噪声维度
    M = 1000
    A = generate_symmetric_matrix(covariate_dim, correlation_strength=0)
    ...
    #best_theta, generated_theta = krr_prediction(kmeans_model, all_means, all_weights, all_covariances, nn_model,x_new,lambda_param=1e-4, length_scale=1.0)
    
    best_theta, generated_theta = linear_regression_predict(kmeans_model, all_means, all_weights, all_covariances,nn_model, x_new)
    ...
    test_points = [np.random.uniform(lowbound, upbound, covariate_dim) for _ in range(20)]


    fake_krr, fake_knn, fake_lr, true1 = [], [], [], []
    for x_new in test_points:
       
        best_theta_knn, generated_theta_knn = knn_predict1(kmeans_model, covariates_points,x_new, gmm_models, n_components1,nn_model,k =5)
        best_theta, generated_theta = krr_prediction(kmeans_model, all_means, all_weights, all_covariances, nn_model,x_new,lambda_param=1e-4, length_scale=1.0)
        best_theta_lr, generated_theta_lr = linear_regression_predict(kmeans_model, all_means, all_weights, all_covariances,nn_model, x_new)
        thetas_true = first_stage1(x_new, 5, 100, eta, covariate_dim)
        thetas_true = thetas_true.reshape(-1,covariate_dim)
        ...
        # print("true best objection",min([objective_function(theta, x_new) for theta in thetas_true]))
        # print("krr generate best objection",objective_function(best_theta, x_new))
        # print("knn generate best objection",objective_function(best_theta_knn, x_new))
        # print("lr generate best objection",objective_function(best_theta_lr, x_new))


        # print("true best objection",min([objective_function(theta, x_new) for theta in thetas_true]))
        # print("generate best objection",objective_function(best_theta, x_new))
        true1.append(min([objective_function(theta, x_new) for theta in thetas_true]))
        fake_krr.append(objective_function(best_theta, x_new))
        fake_knn.append(objective_function(best_theta_knn, x_new))
        fake_lr.append(objective_function(best_theta_lr, x_new))
    ...
    plt.title('Objective Function Comparison')
    ...
    # sns.lineplot(x=range(len(fake_knn)), y=fake_knn, label="generate-GMM-kNN", color='red', marker='*')
    # sns.lineplot(x=range(len(fake_krr)), y=fake_krr, label="generate-GMM-KRR", color='green', marker='o')
    # sns.lineplot(x=range(len(true1 )), y=true1 , label="true ", color='blue', marker='s')
    # sns.lineplot(x=range(len(fake_lr)), y=fake_lr, label="generate-GMM-LR", color='yellow', marker='x')
    
    # plt.xlabel("test covariate index", fontsize=12)
