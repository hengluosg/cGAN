...
def plot_results_subplots(test_points, candidate_solutions, candidate_scores, true_objective_values, global_min_indices, local_min_indices, theta_range, g_values):
    """
    绘制 num_test_points 次结果的局部和全局极值点图像，每个测试点一个子图。

    Parameters:
    - test_points: ndarray, 测试点集合
    - candidate_solutions: list, 每个测试点对应的候选解
    - candidate_scores: list, 每个测试点对应的候选目标值
    - true_objective_values: list, 每个测试点的真实目标值
    - global_min_indices: list, 每个测试点对应的全局最小值索引
    - local_min_indices: list, 每个测试点对应的局部最小值索引
    - theta_range: ndarray, θ 范围
    - g_values: list, 每个测试点的目标函数值 g(θ)
    """
