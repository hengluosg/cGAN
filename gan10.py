...
#     return term1
def objective_function(theta, x):
    term1 = np.sin(np.pi * theta) + 0.2 * (theta - x) ** 2
    return term1
...
def first_stage(n,t, x_dim, K):
    ...
    #X, best_theta #cGAN
    return np.array(X), np.array(objective_values), np.array(best_theta_total)
...
def train_nn_surface_model(X, thetas, objective_values, x_dim, epochs=100, lr=1e-3, batch_size=32):
    ...
    X_repeated = np.repeat(X, t, axis=0)  # 将 X 重复 K 次
    # thetas_flat = thetas.reshape(-1, theta_dim)  # 展平 thetas
    inputs = np.hstack([thetas, X_repeated])  # 拼接 theta 和 x
...
def online_stage(x, G, surface_model, z_dim, M):
    ...
    for theta in candidates:
        ...
        for i in range(theta.shape[0]):
            ...
            nn_input = torch.tensor(np.concatenate([[theta[i]], x]), dtype=torch.float32).unsqueeze(0)
...
def plot_candidates_distribution(candidates, solutions):
    ...
    #candidates = flattened_arr.reshape(-1, 1) 
    ...
    x_range = np.linspace(np.min(candidates), np.max(candidates), len(candidates))
    ...
    x_range = np.linspace(np.min(solutions), np.max(solutions), len(solutions))
...
if __name__ == "__main__":
    ...
    for n in n_values:
        ...
        best_theta = best_theta.reshape(n,t)
        print(X.shape,best_theta.shape)
